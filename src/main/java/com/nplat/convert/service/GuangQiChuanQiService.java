 ...
 * describe 广汽传祺
 * @author HanLing
 * @date 2023-06-19 09:47
 **/

@Service
@Slf4j
public class GuangQiChuanQiService {
                ...
                if (matrixData.size() > 0) {//已经有一行数据了,且第一行数头
                    ...
                    for (int k = 1; k < lineWords.size(); k++) {
                        ...
                        if (lineWords.get(k).getX() > headerWord.getX() && dataSub.size() == 1) {
                            dataSub.add(convertToTimeWordWithPos(lineWords.get(k - 1)));
    ...
    private WordWithPos convertToTimeWordWithPos(WordWithPos wordWithPos) {
        Integer year = Integer.valueOf(wordWithPos.getWord().substring(9, 13));
        if(year.intValue() >= 2021){
            wordWithPos.setWord(wordWithPos.getWord().substring(9, 13) + "-" + wordWithPos.getWord().substring(13, 15) + "-" + wordWithPos.getWord().substring(15, 17));
        }else{
            wordWithPos.setWord("20" + wordWithPos.getWord().substring(9, 11) + "-" + wordWithPos.getWord().substring(11, 13) + "-" + wordWithPos.getWord().substring(13, 15));
        }
        return wordWithPos;
    }
